...
def add_file():
    ...
    files = filedialog.askopenfilenames(title="이미지 파일을 선택하세요", \
        ...
        initialdir=r"C:\Users\hansol\Desktop\new") # 최초에 C:/ 경로를 보여줌 # r을 넣으면 그대로 쓰겠다고 하는 것임
...
def del_file():
    for index in reversed(list_file.curselection()): # reversed를 붙이면 선택 항목이 다 사라짐
        list_file.delete(index)


# 저장 경로 (폴더)
def browse_dest_path():
    folder_selected = filedialog.askdirectory() #폴더 선택하는 창
    if folder_selected == '':
        return
...
def merge_image():
    ...
    try:
        ...
        # zip을 통하여 widths와 heights를 나눌 수 있게 됨 
        ...
        for idx, img in enumerate(images):
            # width 가 원본 유지가 아닐 때는 크기 조정을 해야함
            if img_width > -1:
